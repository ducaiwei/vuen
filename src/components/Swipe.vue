...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      reInitPages() {
        var children = this.$children,pos = this.fixPos;
    ...
    mounted() {
      ...
      this.reInitPages();
      const pos = this.fixPos;
      if(pos != 0){
        const pageWidth = this.pages[pos].clientWidth;
         this.translate(this.pages[pos], this.fixPos*pageWidth,1);
      }

      if (this.auto > 0 && this.autoFlag) {
